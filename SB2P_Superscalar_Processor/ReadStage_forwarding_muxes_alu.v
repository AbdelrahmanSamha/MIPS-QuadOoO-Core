module ReadStage_forwarding_muxes_alu(
   ...
   input [15:0] imm16b_ReadS, 
	...
	wire [31:0] source2operand;
	wire [31:0] immediate;
	assign immediate = {{16{imm16b_ReadS[15]}}, imm16b_ReadS[15:0]};
	
	//in case of an i-type instruction we need to replace the second operand with the sign extended immediate value. 
	mux2_ip immediateorprfmux (.data0x(PRFsource2), .data1x(immediate), .sel(i_type), .result(source2operand));
	...
	mux2_ip  alu_isforwardings2mux (.data0x(source2operand), .data1x(alu_forwarding_lsu_s2), .sel(s2_needforwarding_ReadS), .result(source2));
